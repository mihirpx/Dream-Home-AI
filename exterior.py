...
def exterior_page():
    set_background("app/assets/image0_0.jpg")
    ...
    st.markdown('<div class="center-title">🏡 Exterior Design Generator</div>', unsafe_allow_html=True)

    prompt = st.text_input("Describe the Exterior Design you want:")

    # Centered Generate button using columns
    generate_clicked = False
    col1, col2, col3 = st.columns([1, 0.5, 1])
    with col2:
        if st.button("🎨 Generate Exterior Images"):
            ...
            with st.spinner("Generating..."):
                ...
                for i in range(0, len(images), 2):
                    ...
                    for j in range(2):
                        if i + j < len(images):
                            with cols[j]:
                                st.image(images[i + j], caption=f"Exterior Design {i + j + 1}", use_container_width=True)
